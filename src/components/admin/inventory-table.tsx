...
interface InventoryTableProps {
  ...
  onDeleteItem?: (item: InventoryRow) => void;
}
                ...
                filteredItems.map((item) => {
                  ...
                  return (
                    <tr
                      ...
                      className={`transition cursor-pointer group ${rowClass}`}
                    >
                      {/* SKU */}
                      <td className="py-2.5 px-3 font-mono font-bold text-blue-700 dark:text-blue-400 group-hover:underline">
                        {item.sku}
                      </td>
                      ...
                      <td className="py-2.5 px-3 text-right whitespace-nowrap">
                        <div className="flex items-center justify-end gap-1.5">
                          <button
                            type="button"
                            data-testid="btn-edit-inventory"
                            onClick={(e) => {
                              e.stopPropagation();
                              onEditItem?.(item);
                            }}
                            className="inline-flex items-center gap-1 rounded-md border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-2.5 py-1 text-xs font-semibold text-amber-600 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-950/60 transition active:scale-95 cursor-pointer shadow-2xs"
                            title="Edit Item"
                          >
                          ...
                          <button
                            type="button"
                            data-testid="btn-delete-inventory"
                            onClick={(e) => {
                              e.stopPropagation();
                              onDeleteItem?.(item);
                            }}
                            className="inline-flex items-center gap-1 rounded-md border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-2.5 py-1 text-xs font-semibold text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/60 transition active:scale-95 cursor-pointer shadow-2xs"
                            title="Delete Item"
                          >
