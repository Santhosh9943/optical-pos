# OptixOS — Architecture & System Design Specification
...
1. **Tier 1: Presentation Layer (Client Components & Global State)**:
   ...
   - Rich interactive widgets (Cart, Prescription Grid, Spectacle Builder Wizard, Product Dispatcher, Kanban Board, Reports Ledger).
...
2. **Tier 2: Business & Application Layer (Domain Services & Server Actions)**:
   ...
   - Manages complex multi-step workflows such as atomic checkout transactions, workshop state transitions, and balance settlements.
...
3. **Tier 3: Data & Persistence Layer (PostgreSQL / Neon / Supabase + Drizzle ORM)**:
   ...
   - Fully typed schema governed by **Drizzle ORM** with database-level sequence generators, check constraints, foreign keys, and compound indexes.
...
- **POS Workspace (`src/components/pos/pos-view.tsx`)**:
  ...
  - Hotkey listener handling keyboard ergonomics (`F1` new bill, `F2` product catalog, `F3` search focus, `F5` thermal print, `F10` settlement).
...
#### 5. Financial Accuracy & Arbitrary-Precision Math
...
#### 6. IEEE-754 Safe Dioptric Interval Refinement
...
- Captures over 370 structural nodes, 740 semantic edges, and 19 cohesive communities spanning clinical optical management, transactional order processing, database schemas, and analytics.
...
3. `npm run test:e2e` (Playwright End-to-End browser test suite)

Following validation, `graphify .` is executed to synchronize the architectural knowledge graph.

---

## 7. Master Implementation Roadmap (Phases 1 to 12)
...
| **Phase 9** | **Workshop Glazing & Lens Edging Integration** | 🟡 Next Up | Optical lens edging tickets, lab batch dispatch, outside lab job tracking, automated SMS/WhatsApp alerts upon transition to `READY_FOR_COLLECTION`. |
| **Phase 10** | **Multi-Branch Inventory & Stock Audits** | ⚪ Roadmap | Inter-branch stock transfer orders, in-transit stock ledger, physical stock discrepancy adjustments, barcode scanner batch intake, automated reorder threshold triggers. |
| **Phase 11** | **Clinical Appointments & Patient Recalls** | ⚪ Roadmap | Optometrist appointment scheduling calendar, exam room slot booking, annual eye examination recall reminders via WhatsApp/SMS, expanded clinical notes & visual acuity charts. |
| **Phase 12** | **Multi-Tenant SaaS Hardening & Enterprise ERP** | ⚪ Roadmap | Row-Level Security (RLS) policies for multi-tenant optical chains, consolidated corporate tax reporting, Indian GST 2.0 inverted duty credit reconciliation (HSN 9001 vs 9003/9004), immutable audit log stream. |
